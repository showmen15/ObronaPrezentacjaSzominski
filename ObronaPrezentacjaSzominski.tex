...
\note{
\begin{itemize}
	\item Co stanowiło inspiracje do stworzenia algorytmów koordynacji robotów, przestawię ich główne cechy, zaprezentuję najważniejsze  osiągnięcia pracy.
...
\item Na koniec podam wnioski i kierunki dalszych rozwoju. 
\end{itemize}
...
\item W relacjach zawodowych widoczna jest najlepiej organizacja grupy społecznej.  Osoby zajmujące wysokie stanowiska kierowniczych, obdarzane są przez osoby na niższych stanowiskach respektem. Respekt nie wynika tylko z pozycji, jaką zajmuje dany osobnik w hierarchii.	Zjawisko to jest znacznie bardziej szerokie i właśnie ono, dzięki swojej złożoności stonowało inspiracje do stworzenia algorytmu koordynacji ruchu robotów mobilnych.
\end{itemize}
}
...
\item Teza rozprawy sformułowana jest następująco:
...
Jeżeli robot wykryje robota nadjeżdżającego z prawej strony, to w zaistniałej sytuacji zobligowany jest do ustąpienia mu pierwszeństwa. Robot zmienia swoją dotychczasową trajektorię ruchu starając się ominąć robota zbliżającego się z prawej strony (linia: 6). Gdy roboty wzajemnie się wyminą, robot wyznacza trajektorię do celu (linia: 8), aby powrócić do wykonywania powierzonego zadania.
...
W celu zewryfikowania poprawności działania algorytmów przeprawadzono testy w następująych śrowowiskach i ustawieniach robotów:
...
\begin{itemize}
	...
	\item Skrzyżowanie równorzędne.
	\item Skrzyżowanie typu 8 -- gdzie roboty musza wyznaczć pierwszeństwo ruch.
	\item Mijanka -- gdzie roboty musza wykorzystać mijankę do wzajemego wyminięcia się.	
\end{itemize}

Zadaniem robot jest przejechanie od punktu startowego do punktu docelowego (oznaczone literą X) omijając inne roboty oraz przeszkody statyczne.
}
