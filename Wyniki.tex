...
\note{
\begin{itemize}
	\item Dla ustawienie 1-2 naslepszy łączny czas przjzdu robotów uzyskała algorytm PR. Jako drugi poradził sobie z zadaniem algorytm RVO. Algorytmy R+ oraz R osiągneły podobne rezultaty.

	\item Przy wiekszej liczbie robotów sytuacja znacząco się zmienia. Algotytm R osiągął najlepszy czas jako drugi algorytm R+. Trzecie miejsce to algrotym RVO zaś najgorzej poradziła sobie metoda PR.
\end{itemize}
}
...
\begin{itemize}
	\item Metoda RVO oraz PR nie poradziły sobie zarówni w ustawieniu 1-H2 oraz 9-H8. Roboty wzajemnie zakleszczyły się w przed przejściem.
	
	\item W obu ustawieniach wyraźnie zaznacza się trand iż metoda PF, PF+ jest nieco lepsza niż metoda R, R+
	
	\item Algoryrmy R+ oraz PF+ osiągneły gorsze wyniki niż metody R oraz R+
\end{itemize}

Litera H – oznaczon w którym pomieszczeniu roboty miały wyższe bazowy współczynnik respektu.
...
\note{
	...
	\begin{itemize}
		\item Również i w wąskim korytarzu metody RVO i PR nie poradziły sobie z zadniem.
		\item Metoda R osiągneła znacznie lepszy czas przejazdu niż metoda R+
	\end{itemize}
}
...
\note{
W celu zweryfikowania poprawności działania algorytmów wykorzystano platformę Czterokołową Autonomiczną Platformę Mobilną CAPO zbudowaną na AGH.

Platforma:

\begin{itemize}
	\item zasilana jest z dwóch baterii litowo-polimerowych, 
	\item napędzane jest przez 4 silniki kontrolwane przez dwa kontrolery RoboClaw 2 x 15A firmy BasicMicro,
	\item które kontroluje komputer starujący PandaBoard ES.
\end{itemize}
...
\begin{itemize}
	...
	\item Metody R oraz R+ osiągneły podobny rezultat.
...
\note{
\begin{itemize}
	\item Metody RVO oraz PR ze względu na brak rezultatów w symulacji nie byłu uruchamiana na rzeczywistych robotach.
	\item Nalepiej poradziła sobie metoda PF+ oraz PF.
	\item Algorytm R osiągnał trzeci wynik.
	\item Najgorzej z zaniem poradziła sobie metoda R+.
\end{itemize}
